...
async function main() {
  ...
  // Do not revive sessions explicitly disconnected by the owner. Reviving
  // them can create two sockets for the same WhatsApp number and WhatsApp
  // then drops both with "Connection Failure".
  for (const connection of connections.filter((item) => item.status !== 'disconnected')) {
