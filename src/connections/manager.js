...
class ConnectionManager {
  ...
  async disconnect(id) { const sock = this.sockets.get(id); if (sock) { try { sock.end(undefined); } catch {} this.sockets.delete(id); } const store = readStore(); const item = store.connections.find((c) => c.id === id); if (item) { item.status = 'disconnected'; item.updatedAt = Date.now(); writeStore(store); addLog({ level: 'info', type: 'connection.disconnected', connectionId: id, message: `${item.name} disconnected by user` }); } }
